 ...
 * ----------------------------------------
 * A graph where "degrees of separation" grows logarithmatically with graph
 * size. Think: "six degrees of Kevin Bacon" - but with log(n), not 6.
 *
...
public:
  ...
  SearchStrategy getSearchStrategy() const { return search_strategy_; }
...
//                    if (candidate_id == 0) continue;
  ...
  // Experimental search that expands one node across its available layers in a
  // single traversal.
  template <bool bare_bone_search = true, bool collect_metrics = false>
  std::priority_queue<std::pair<dist_t, tableint>,
                      std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
  searchBaseLayerSTVF(
    ...
    while (!candidate_set.empty()) {
      ...
      // Track the absolute best distance found from this node's neighbors
      dist_t absolute_best_dist_for_node = candidate_dist;

      for (int level = element_levels_[current_node_id]; level >= 0; level--) {
        // OPTIMIZATION: Level 0 Fast-Path
        int *data = (level == 0)
                        ? (int *)get_linklist0(current_node_id)
                        : (int *)get_linklist_at_level(current_node_id, level);
        ...
        // OPTIMIZATION: Prefetch first elements of visited list
#ifdef USE_SSE
        _mm_prefetch((char *)(visited_array + *(data + 1)), _MM_HINT_T0);
        _mm_prefetch((char *)(visited_array + *(data + 1) + 64), _MM_HINT_T0);
#endif

        bool found_closer_neighbor_this_layer = false;

        for (size_t j = 1; j <= size; j++) {
          int candidate_id = *(data + j);

          // OPTIMIZATION: Prefetch NEXT vector's visited flag and data
#ifdef USE_SSE
          if (j + 1 <= size) {
          ...
          // HEURISTIC FIX: Are we getting closer to the target?
          if (dist < absolute_best_dist_for_node) {
            absolute_best_dist_for_node = dist;
            found_closer_neighbor_this_layer = true;
          }
          ...
          bool flag_remove_extra = (!bare_bone_search && stop_condition)
        ...
        // HEURISTIC CHECK: If this layer didn't push us closer to the target
        // than we already were, stop drilling down into denser lower layers.
        if (!found_closer_neighbor_this_layer) {
          break;
        }
      }
    }
  ...
  searchStopConditionClosest(const void *query_data,
    ...
    if (search_strategy_ == VIRTUAL_FLATTENED) {
      std::priority_queue<std::pair<dist_t, tableint>,
          ...
          top_candidates;
      top_candidates = searchBaseLayerSTVF<false>(
          enterpoint_node_, query_data, 0, isIdAllowed, &stop_condition);
